...
public class BemCandidatoResource {
    ...
    public ResponseEntity<Page<BemCandidato>> findPage(@RequestParam(value = "page", defaultValue = "0") Integer page,
        ...
        return ResponseEntity.ok(service.findPage(page, linesPerPage, orderBy, direction).orElseThrow(() ->
                new ObjectNotFoundException("Nenhum objeto foi encontrado! Tipo: " + BemCandidato.class.getName())));
    }
    ...
    public ResponseEntity<BemCandidato> find(@PathVariable Long id) {
        return ResponseEntity.ok(service.find(id).orElseThrow(() ->
                new ObjectNotFoundException("Objeto não encontrado! Id: " + id + ", Tipo: " + BemCandidato.class.getName())));
    }
